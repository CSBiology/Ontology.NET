...
type Protocol internal (
    ...
    member this.Version 
        ...
        and set(version : string) =
            CvParam.fromValue (Terms.version |> CvTerm.create) version
...
[<Tests>]
let cvContainerTests = testList "CvContainerTests" [

    testList "correctlyImplementsICvBase" [

        let container = CvContainer(testTerm  |> CvTerm.create)
