...
type CvParam(cvAccession : string, cvName : string, cvRef : string, paramValue : ParamValue, attributes : IDictionary<string,IParam>) =
    ...
    static member getParamValue (cvp: CvParam) = 
        Param.getParamValue cvp
    ...
    static member getValueAsString (cvp: CvParam) = 
        Param.getValueAsString cvp

    /// Returns the value of the Param as int if possible, else fails.
    static member getValueAsInt (cvp: CvParam) = 
        Param.getValueAsInt cvp

    /// Returns the value of the Param as a term.
    static member getValueAsTerm (cvp: CvParam) = 
        Param.getValueAsTerm cvp

    /// Returns the CvParam's value's accession if it exists. Else returns None.
    static member tryGetValueAccession (cvp: CvParam) = 
        Param.tryGetValueAccession cvp

    /// Returns the CvParam's value's reference if it exists. Else returns None.
    static member tryGetValueRef (cvp: CvParam) = 
        Param.tryGetValueRef cvp

    /// Returns the CvParam's value's CvUnit if it exists. Else returns None.
    static member tryGetCvUnit (cvp: CvParam) : CvUnit option = 
        Param.tryGetCvUnit cvp

    /// Returns the CvParam's value's CvUnit's value if it exists. Else returns None.
    static member tryGetCvUnitValue (cvp: CvParam) = 
        Param.tryGetCvUnitValue cvp

    /// Returns the CvParam's value's CvUnit's term name if it exists. Else returns None.
    static member tryGetCvUnitTermName (cvp: CvParam) = 
        Param.tryGetCvUnitTermName cvp

    /// Returns the CvParam's value's CvUnit's term accession if it exists. Else returns None.
    static member tryGetCvUnitTermAccession (cvp: CvParam) = 
        Param.tryGetCvUnitTermAccession cvp

    /// Returns the CvParam's value's CvUnit's term reference if it exists. Else returns None.
    static member tryGetCvUnitTermRef (cvp: CvParam) = 
        Param.tryGetCvUnitTermRef cvp

    /// Maps the value of the CvParam by appyling the given function.
    static member mapValue (f : ParamValue -> ParamValue) (cvp: CvParam) = 
        Param.mapValue f cvp :?> CvParam

    /// Tries to map the value of the CvParam by appyling the given function. If successful, returns Some, else returns None.
    static member tryMapValue (f : ParamValue -> ParamValue option) (cvp: CvParam) = 
        Param.tryMapValue f cvp 
        |> Option.map (fun v -> v :?> CvParam)

    /// Tries to add a name to the CvParam. If successful, returns Some, else returns None.
    static member tryAddName (name : string) (cvp: CvParam) = 
        Param.tryAddName name cvp
        |> Option.map (fun v -> v :?> CvParam)

    /// Tries to add an accession to the CvParam. If successful, returns Some, else returns None.
    static member tryAddAccession (acc : string) (cvp: CvParam) = 
        Param.tryAddAccession acc cvp
        |> Option.map (fun v -> v :?> CvParam)

    /// Tries to add a reference to the CvParam. If successful, returns Some, else returns None.
    static member tryAddReference (ref : string) (cvp: CvParam) = 
        Param.tryAddReference ref cvp
        |> Option.map (fun v -> v :?> CvParam)

    /// Tries to add a CvUnit to the CvParam. If successful, returns Some, else returns None.
    static member tryAddUnit (unit : CvUnit) (cvp: CvParam) = 
        Param.tryAddUnit unit cvp
        |> Option.map (fun v -> v :?> CvParam)

    /// Returns the ID of the CV item.
    static member getCvAccession (cvp: CvParam) = 
        Param.getCvAccession cvp

    /// Returns the name of the CV item.
    static member getCvName (cvp: CvParam) = 
        Param.getCvName cvp

    /// Returns the reference of the CV item.
    static member getCvRef (cvp: CvParam) = 
        Param.getCvRef cvp

    /// Returns the full term of the CV item.
    static member getTerm (cvp: CvParam) = 
        Param.getTerm cvp

    /// Returns true if the given term matches the term of the CV item.
    static member equalsTerm (term : CvTerm) (cvp: CvParam) = 
        Param.equalsTerm term cvp

    /// Returns true if the terms of the given CvParam items match.
    static member equals (cvp1 : CvParam) (cvp2 : CvParam) = 
        Param.equals cvp1 cvp2

    /// Returns true if the names of the given CvParam items match.
    static member equalsName (cvp1 : CvParam) (cvp2 : CvParam) = 
        Param.equalsName cvp1 cvp1
    ...
    /// Creates a CvParam from a category and a value coming from a CvTerm.
    static member fromCategory (category : CvTerm) (term : CvTerm) =
        CvParam(category, ParamValue.CvValue term)

    /// Creates a CvParam from a category, a simple value and a CvUnit value.
    ...
    /// Returns the attributes of a CvParam as a sequence.
