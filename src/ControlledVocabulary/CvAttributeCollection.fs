...
type CvAttributeCollection(attributes : IDictionary<string,IParam>) =
    ...
    /// Adds an IParam as an attribute. Fails if an attribute with the same key already exists.
    member this.AddAttribute (param : IParam) =
        Dictionary.addInPlace (param |> CvBase.getCvName) param this
        |> ignore

    /// Adds an IParam as an attribute. Does not fail if an attribute with the same key already exists.
    ...
    /// Returns true if it implements a CvAttributeCollection and the attribute CvTerm exists.
