...
/// Ontology containing OBO Terms and OBO Typedefs (OBO 1.2).
type OboOntology =
...
type OboOntologyHeaderTags = 
    ...
    /// </summary>
    /// <param name="formatVersion">Gives the OBO specification version that this OboOntology uses. This is useful if tag semantics change from one OBO specification version to the next.</param>
    /// <param name="dataVersion">Gives the version of the current OboOntology. This gets translated to versionIRI in OWL.</param>
    /// <param name="ontology">The ID space of this OboOntology. This should correspond to to the ID prefix of the terms that belong to that ontology, translated to lowercase. For GO, the value of this field will be "go". For the cell ontology (i.e. the ontology that contains CL:0000001), the value will be "cl". If the obo document contains some alternative cut or extension of the ontology (for example, a GO slim, or an ontology merged with another), then the ontology should be of form "X/Y", where X is the basic ontology name, and Y identifies the cut. For example go/gosubset_prok. A URI is also permitted in here. In the translation to OWL, the usual default prefix rules will apply, with the ".owl" suffix. E.g. "go" will be treated as "http://purl.obo-library.org/obo/go.owl".</param>
    /// <param name="date">The current date in dd:MM:yyyy HH:mm format (note: for historic reasons, this is NOT a ISO 8601 date, as is the case for the creation-date field).</param>
    /// <param name="savedBy">The username of the person to last save this OboOntology. The meaning of "username" is entirely up to the application that generated the file.</param>
    /// <param name="autoGeneratedBy">The program that generated the OboOntology.</param>
    /// <param name="subsetdefs">A description of a term subset. The value for this tag should contain a subset name, a space, and a quote enclosed subset description.</param>
    /// <param name="imports">A url or ontology ID referencing another OBO document.</param>
    /// <param name="synonymtypedefs">A description of a user-defined synonym type. The value for this tag should contain a synonym type name, a space, a quote enclosed description, and an optional scope specifier.</param>
    /// <param name="idspaces">A mapping between a "local" ID space and a "global" ID space. The value for this tag should be a local idspace, a space, a URI, optionally followed by a quote-enclosed description.</param>
    /// <param name="defaultRelationshipIdPrefix">Any relationship lacking an ID space will be prefixed with the value of this tag.</param>
    /// <param name="idMappings">Maps a Term or Typedef ID to another Term or Typedef ID. The main reason for this tag is to increase interoperability between different OBO ontologies.</param>
    /// <param name="remarks">General comments for this file. This tag is differentiated from a ! comment in that the contents of a remark tag are guaranteed to be preserved by a parser.</param>
    /// <param name="treatXrefsAsEquivalents">The value for this tag should contain an ID Space.</param>
    /// <param name="treatXrefsAsGenusDifferentias">The value for this tag should contain an ID Space followed by a relation and then a class filler.</param>
    /// <param name="treatXrefsAsRelationships">The value for this tag should contain an ID Space followed by a relation ID.</param>
    /// <param name="treatXrefsAsIsAs">The value for this tag should contain an ID Space.</param>
    /// <param name="relaxUniqueIdentifierAssumptionForNamespaces">The value for this tag should be a namespace. By default, an OBO namespace (note: not ID-space) partitions all the entities such that no two entities belonging to the same namespace may be equivalent. This header tag relaxes this assumption.</param>
    /// <param name="relaxUniqueLabelAssumptionForNamespaces"></param>
